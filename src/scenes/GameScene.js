...
export class GameScene extends Phaser.Scene {
    ...
    handleVictory(winner) {
        ...
        maskShape.fillRect(cx - boardSize/2, cy - boardSize/2, boardSize, boardSize);
        const boardMask = maskShape.createGeometryMask();

        // 2. Victory Container (All victory elements will be here)
        const vicContainer = this.add.container(0, 0).setDepth(1000);
        vicContainer.setMask(boardMask);

        // 3. Dark Overlay
        const overlay = this.add.graphics();
        overlay.fillStyle(0x000000, 0.7); // Darken the board
        overlay.fillRect(cx - boardSize/2, cy - boardSize/2, boardSize, boardSize);
        overlay.setAlpha(0);
        vicContainer.add(overlay);

        // 4. Generate Starburst (Spinning Sun Rays) for the winner
        if (!this.textures.exists('starburst')) {
            const rayGraphics = this.add.graphics();
            rayGraphics.fillStyle(0xffffff, 1);
            for (let i = 0; i < 12; i++) {
                rayGraphics.slice(250, 250, 250, Phaser.Math.DegToRad(i * 30), Phaser.Math.DegToRad(i * 30 + 15), false);
                rayGraphics.fillPath();
            }
            rayGraphics.generateTexture('starburst', 500, 500);
            rayGraphics.destroy();
        }
        ...
        const titleText = this.add.text(cx, cy - 40 * this.mainScale, winnerText, {
            ...
            stroke: '#000000',
            strokeThickness: 10,
            shadow: { offsetX: 0, offsetY: 5, color: '#000', blur: 0, stroke: true, fill: true }
        }).setOrigin(0.5).setScale(0); // Starts at size 0
        vicContainer.add(titleText);
        ...
        const emitter = this.add.particles(cx, cy, 'particle_dot', {
            ...
            gravityY: 600, // Pull down
            lifespan: { min: 1500, max: 2500 },
            scale: { start: 1.5, end: 0 }, // Confetti shrinks before disappearing
            alpha: { start: 1, end: 0 },
            tint: confettiColors,
            emitting: false, // Wait for our command
            frequency: -1 // Manual bursts mode
        });
        ...
        pawnColors.forEach((pColor, i) => {
            ...
            const pX = cx - 90*this.mainScale + (i * 60*this.mainScale);
            const pY = cy + 130*this.mainScale;
            pawn.setPosition(pX, pY);
            pawn.setAlpha(0);
            pawn.setScale(this.mainScale * 1.5);
            vicContainer.add(pawn);
            ...
            this.tweens.add({
                ...
                duration: 400,
                yoyo: true,
                repeat: -1, // Keep jumping forever
                ease: 'Sine.easeOut',
                delay: 1000 + (i * 150) // Cascade jumping effect
            });
        });

        // 9. Trigger The Animations Sequence
        
        // Darken Board
        this.tweens.add({ targets: overlay, alpha: 1, duration: 500 });

        // Rays Glowing and Spinning
        this.tweens.add({
            targets: rays,
            alpha: 0.4, // Gorgeous glow
            angle: 360,
            duration: 10000,
            repeat: -1
        });
        ...
        this.tweens.add({
            ...
            delay: 300,
            onStart: () => {
                // FESTIVE CONFETTI BARRAGE
                emitter.explode(150); // Massive initial burst
                
                // Keep shooting confetti periodically
                this.time.delayedCall(1500, () => emitter.explode(80));
                this.time.delayedCall(3000, () => emitter.explode(80));
                this.time.delayedCall(4500, () => emitter.explode(100));
            }
        });

        // Subtext floating up
        this.tweens.add({
            targets: [subTextBg, subText],
            alpha: 1,
