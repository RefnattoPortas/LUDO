...
export class GameScene extends Phaser.Scene {
    ...
    drawBase(graphics, x, y, color, colorName) {
        ...
        // Main colored area (White background)
        graphics.fillStyle(0xffffff, 1);
        ...
        // Internal Area Lighter color
        graphics.fillStyle(color, 0.05); // subtle fill
            ...
            this.animatePath(color, index, result.oldPos, result.newPos, () => {
                if (result.captured && result.captured.length > 0) {
                    ...
                    result.captured.forEach(cap => {
                        ...
                        // Ghost effect: fade out and scale up
                        this.tweens.add({
                            targets: capSprite,
                            alpha: 0,
                            scale: 2,
                            ...
                            onComplete: () => {
                                // Reappear in base
                                this.updateAllPiecePositions(true); // reset position instantly while invisible
                                this.tweens.add({
                                    targets: capSprite,
                                    alpha: 1,
                                    scale: 1,
