...
export class GameScene extends Phaser.Scene {
    ...
    onOnlineUpdate(state) {
        ...
        // Log clear for debugging
        console.log(`[REALTIME] Turn: ${state.current_turn}, Roll: ${state.last_dice_roll}`);
        ...
        if (isDifferentTurn || isDifferentDice || isDifferentPieces) {
            console.log(`[REALTIME] Syncing local state with server...`);
            
            this.logic.turn = state.current_turn;
            this.logic.diceRoll = state.last_dice_roll;
            this.logic.pieces = state.pieces;
            
            // Critical: Update the internal GameState
            // If roll is 0, we are waiting for someone to roll. If > 0, someone rolled and we are waiting for move.
            ...
            if (this.logic.diceRoll === 0) {
                ...
                // If the update came from another player, animate the dice show
