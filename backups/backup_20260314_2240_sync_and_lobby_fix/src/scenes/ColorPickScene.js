...
export class ColorPickScene extends Phaser.Scene {
    ...
    startGame(activeColors) {
        const userColor = activeColors[0];
        let finalActivePlayers = [userColor];
        ...
        if (this.mode === 'IA') {
            ...
            if (this.playerCount === 2) {
                // Modo 2 jogadores: Cor oposta
                finalActivePlayers.push(opposites[userColor]);
            } else if (this.playerCount === 3) {
                // Modo 3 jogadores: Próximas duas cores na sequência
                const userIdx = allColors.indexOf(userColor);
                const next1 = allColors[(userIdx + 1) % 4];
                const next2 = allColors[(userIdx + 2) % 4];
                finalActivePlayers.push(next1, next2);
            } else if (this.playerCount === 4) {
                // Modo 4 jogadores: Todas as cores, começando pelo usuário
                const userIdx = allColors.indexOf(userColor);
                for (let i = 1; i < 4; i++) {
                    finalActivePlayers.push(allColors[(userIdx + i) % 4]);
                }
            }
        } else {
            finalActivePlayers = activeColors;
        }
